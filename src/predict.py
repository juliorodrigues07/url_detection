...
DETECTIONS = [
    {
        'URL': 'https://bujhanginamfb.github.io/taelasos/update-recovry/',
        'Type': 'Phishing',
        'Probability': '99.99%',
        'Algorithm': 'XGBoost'
    },
...
def classify_url(url, algorithm):
    ...
    return prediction, round(probability * 100, 2), True
...
def cli():
    ...
    url = str(argv[2])
    prediction, reliability, check = classify_url(url, algorithm)
...
def url_request():
    ...
    if len(post_data) == 2:
        ...
        prediction, reliability, check = classify_url(url, alg)
